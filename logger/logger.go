...
var (
	strExclude = []string{
		"password", "base64", "npwp", "phone", "nik", "ktp", "gaji", "othr", "qr", "cif", "tokens", "imageQr", "logo",
		"token",
	}
...
func Make(c echo.Context, payload interface{}) *logrus.Entry {
	...
	isFileImage := checkIsFileImage(pl)
	if payload != nil && !isFileImage {
		payloadExcluder(&pl)
...
func checkIsFileImage(resp []byte) bool {
	...
	for magic := range magicTable {
		if strings.HasPrefix(string(resp), magic) {
			return true
		}
	}
